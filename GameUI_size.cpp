...
static VERTEX_2D				g_vertexWk_num[UI_SIZE_DIGIT][NUM_VERTEX];	// 頂点情報格納ワーク

static LPDIRECT3DTEXTURE9		g_pD3DTexture_text = NULL;			// テクスチャへのポリゴン
static VERTEX_2D				g_vertexWk_text[NUM_VERTEX];	// 頂点情報格納ワーク


static D3DXVECTOR3				g_posUI_size;						// ポリゴンの移動量

static int						g_UI_size;						// スコア
...
HRESULT InitUI_size(int type)
{
	...
	if (type == 0)
	{
		...
		D3DXCreateTextureFromFile(pDevice,		// デバイスのポインタ
			UI_SIZE_TEXT_GAME_FILE,					// ファイルの名前
...
{
	int i;
	float habaX = UI_SIZE_SIZE_W;	// 数字の横幅
